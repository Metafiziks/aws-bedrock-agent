...
Stable memory eval cases live in tests/memory_eval_cases.json so doc-case
generation can safely overwrite tests/eval_cases.json without dropping memory
coverage.
"""
...
MEMORY_EVAL_CASES_PATH = Path(__file__).parent.parent / "tests" / "memory_eval_cases.json"
...
def check_memory_thresholds(cases: list[dict]) -> list[str]:
    failures = []
    for case in cases:
        if not is_memory_case(case) or case.get("skipped"):
            continue
        threshold = float(case.get("pass_keyword_recall", 1.0))
        value = case.get("scores", {}).get("keyword_recall", 0.0)
        if value < threshold:
            failures.append(
                f"{case['id']} memory keyword_recall: {value:.4f} < {threshold:.4f} threshold"
            )
    return failures
...
def load_eval_cases() -> list[dict]:
    ...
    if MEMORY_EVAL_CASES_PATH.exists():
        cases.extend(json.loads(MEMORY_EVAL_CASES_PATH.read_text()))
    return cases
...
def run_memory_case(case: dict) -> dict:
    ...
    first_turn = turns[0]
    recall_turn = turns[-1]
    close_message = case.get("close_message", "End this session and save the conversation memory summary.")
    ...
    print(f"seeding memory, ending session for summary, then waiting/retrying recall (memoryId={memory_id}) ... ", end="", flush=True)
    first_answer, _, first_latency_ms, first_body = call_agent(
        first_turn["message"],
        session_id=first_session_id,
        memory_id=memory_id,
    )
    close_answer, _, close_latency_ms, close_body = call_agent(
        close_message,
        session_id=first_session_id,
        memory_id=memory_id,
        end_session=True,
    ...
    answer, citations, recall_latency_ms, recall_body, scores, best_attempt = best
    memory_passed = scores["keyword_recall"] >= float(case.get("pass_keyword_recall", "1.0"))
    result.update({
        "answer": answer,
        "citations": citations,
        "latency_ms": round(first_latency_ms + close_latency_ms + recall_latency_ms, 1),
        "scores": scores,
        ...
        "memory": {
            ...
            "close_response_memory_id": close_body.get("memoryId"),
    ...
    print(f"kw={scores['keyword_recall']:.2f} attempts={attempts_run} {result['latency_ms']:.0f}ms")
    return result
...
def main():
    ...
    failures = []
    if not args.no_judge:
        failures.extend(check_thresholds(summary, THRESHOLDS))
    failures.extend(check_memory_thresholds(results))
